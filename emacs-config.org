...
Test for native Emacs Lisp compilation (with =libgccjit=) and native JSON parsing (with =libjansson=). See [[https://www.masteringemacs.org/article/speed-up-emacs-libjansson-native-elisp-compilation][here]].
...
When Emacs starts in daemon mode, it won't always enable our custom themes. So we specify a function that handles everything to do with our custom themes, and we ensure it gets run once the daemon has finished loading. See [[https://stackoverflow.com/a/23668935][here]] for more info.
