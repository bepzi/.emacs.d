...
Native compilation will likely become part of vanilla Emacs starting in version 28. In the event that we do support it, we'll enable deferred compilation mode so that we don't spend a ton of time AOT compiling everything.
...
When Emacs starts in daemon mode, it won't reliably enable our custom themes. For example, when you open =emacsclient=, I see my custom color theme, but the code that configures =whitespace-mode='s style doesn't get the right colors, because when the daemon starts, Emacs somehow thinks the background color is pure black. So we specify a function that handles everything to do with our custom themes, and we ensure it gets run only after the daemon has finished loading. See [[https://stackoverflow.com/a/23668935][here]] for more info.
  ...
  (defun my/apply-themes-and-styles ()
    ...
    (with-eval-after-load 'whitespace
      (let ((lighter-background (my/lighten-color (face-attribute 'default :background))))
        (set-face-attribute 'whitespace-indentation      nil :background nil :foreground lighter-background)
        (set-face-attribute 'whitespace-space-after-tab  nil :background nil :foreground lighter-background)
        (set-face-attribute 'whitespace-space-before-tab nil :background nil :foreground lighter-background)
        (set-face-attribute 'whitespace-space            nil :background nil :foreground lighter-background)
        (set-face-attribute 'whitespace-tab              nil :background nil :foreground lighter-background)
        (set-face-attribute 'whitespace-newline          nil :background nil :foreground lighter-background)
        (set-face-attribute 'whitespace-empty            nil :background nil :foreground lighter-background)
    ...
    (with-eval-after-load 'org
      (progn
        (let* ((variable-font (my/get-installed-font
                               '("ETBembo" "Liberation Serif" "Source Serif Pro" "Source Sans Pro" "Segoe UI")))
               (variable-tuple `(:font ,variable-font))
        ...
        (when (equal (face-attribute 'variable-pitch :family) "ETBembo")
          (set-face-attribute 'variable-pitch nil :height 135)))))
#+end_src

If we're loading in daemon mode, defer loading the themes; otherwise, just do it immediately.
...
Theme Org-mode so that it's more WYSIWYG and writing-oriented. See [[https://zzamboni.org/post/beautifying-org-mode-in-emacs/][here]].

In particular, we use a variable-pitched font for the basic text, and a fixed-pitch font for code and attributes.
...
Set up [[https://github.com/bastibe/org-journal][org-journal]].

Helpful info from the documentation:

#+begin_quote
Doing =M-x org-journal-new-entry= (or =C-c C-j=) will immediately create a journal directory in the default path... open or create a file in =org-journal-mode=, and insert a template for a new journal entry.
...
I have the prefix set to =C-c j= so that it doesn't conflict with vanilla Org mode.

#+begin_src elisp
  (use-package org-journal
    :if (boundp 'secrets/org-journal-dir)
    :defer t
    :init (setq org-journal-prefix-key "C-c j ")
    :config (setq org-journal-dir secrets/org-journal-dir
